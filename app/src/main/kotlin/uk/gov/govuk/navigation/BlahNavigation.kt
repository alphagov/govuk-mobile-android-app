...
@Singleton
internal class BlahNavigation @Inject constructor(
    ...
    fun onDifferentUserLogin(topicsInitSuccess: Boolean) {
        _launchRoutes.clear()
    ...
    fun onNext(navController: NavController) {
        ...
        if (_launchRoutes.isNotEmpty()) {
            val route = if (_launchRoutes.size > 1) _launchRoutes.pop() else _launchRoutes.peek()
