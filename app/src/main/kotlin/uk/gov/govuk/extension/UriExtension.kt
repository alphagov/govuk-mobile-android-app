 ...
 */
internal fun Uri.getUrlParam(allowedUrls: List<String>, allowedUrlParams: List<String>): Uri? {
    fun Uri.isInAllowedList(urls: List<String>): Boolean {
        val url = "${this.scheme}://${this.host}"
        return urls.contains(url)
    }

    if (!this.isInAllowedList(allowedUrls)) {
        return null
    }
